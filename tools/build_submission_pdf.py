...
FONT_SHA256 = {
    # Keep these values synchronized with docs/fonts/SOURCE.md so a substituted
    # font cannot silently change the PDF.
    "NotoSansSC-Regular.ttf": "198D40E70EE7CC1342EA436D2DFDF76E5AC13A9765DCD5F2A3A7655CF07F72F9",
    "NotoSansSC-Bold.ttf": "37BA4C2D255E3491D96988E5702DA52FE2F5665F375E8FEECAFA0102350CBFF2",
}
...
def register_fonts() -> None:
    for family, filename in FONT_FILES.items():
        path = FONTS / filename
        if not path.is_file():
            raise FileNotFoundError(f"Missing redistributable PDF font: {path}")
        actual = sha256_file(path)
        expected = FONT_SHA256[filename]
        if actual != expected:
            raise RuntimeError(f"PDF font hash mismatch for {filename}: {actual}")
...
def style(name: str, **overrides) -> ParagraphStyle:
    values = {"fontName": "NotoSansSC", "fontSize": 9.8, "leading": 16, "textColor": GREEN, "spaceAfter": 7, "wordWrap": "CJK"}
...
def build(output: Path = OUTPUT) -> Path:
    ...
    h1 = style("H1", fontName="NotoSansSC-Bold", fontSize=20, leading=29, spaceAfter=10)
    h2 = style("H2", fontName="NotoSansSC-Bold", fontSize=12.5, leading=19, textColor=GREEN_2, spaceBefore=4)
    body = style("Body")
    small = style("Small", fontSize=8.1, leading=12.5, textColor=MUTED)
    white = style("White", fontSize=10.5, leading=18, textColor=CREAM)
    metric = style("Metric", fontName="NotoSansSC-Bold", fontSize=19, leading=24, alignment=TA_CENTER)
    metric_label = style("MetricLabel", fontSize=7.8, leading=11, textColor=MUTED, alignment=TA_CENTER)
    tag_style = style("Tag", fontName="NotoSansSC-Bold", fontSize=8.3, leading=11, alignment=TA_CENTER)
    ...
    doc = BaseDocTemplate(str(output), pagesize=A4, rightMargin=18 * mm, leftMargin=18 * mm, topMargin=17 * mm, bottomMargin=19 * mm, title="净界 AI 内容工厂 v2 Agent 工作台补充材料", author="净界 AI 内容工厂")
    ...
    story = [
        ...
        paragraph("描述目标 → 三选一授权 → Agent 自动推进 → 两处人工确认 → 成片与证据包", white), Spacer(1, 33 * mm),
        ...
        paragraph("人工责任", h2), paragraph("主流程每道门禁只保留一个明确的确认操作；详细页仍可逐条查看、批准、拒绝或退回 finding。Agent 只把严格反证审核判定为可用的 finding 写入审批记录，最终脚本仍须用户亲自放行。", body), PageBreak(),
        ...
        metric_table([("3.12 / 3.14", "Python CI"), (str(test_count), "Python 测试"), (str(package_count), "能力包"), ("0", "npm 漏洞")]), Spacer(1, 6 * mm),
        ...
        Spacer(1, 8 * mm), Table([[paragraph("v2 DeepSeek 证据包已在用户亲自完成两次审批后生成。自动测试证明机制，用户审批证明人工事实；旧运行快照继续只作为 legacy 对照。", style("Callout", fontName="NotoSansSC-Bold", fontSize=11.5, leading=19))]], colWidths=[174 * mm], style=TableStyle([("BACKGROUND", (0, 0), (-1, -1), CREAM), ("BOX", (0, 0), (-1, -1), 0.8, LINE), ("TOPPADDING", (0, 0), (-1, -1), 12), ("BOTTOMPADDING", (0, 0), (-1, -1), 12)])),
    ]
    doc.build(story)
    return output
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build the deterministic v2 competition proposal PDF.")
    parser.add_argument("--output", type=Path, default=OUTPUT)
    parser.add_argument("--check", action="store_true", help="Rebuild in a temporary directory and compare with --output.")
    parser.add_argument("--verify-deterministic", action="store_true", help="Build twice and require byte-identical PDFs.")
    args = parser.parse_args()

    if args.check or args.verify_deterministic:
        with tempfile.TemporaryDirectory(prefix="shiyi-pdf-") as temp_name:
            temp_dir = Path(temp_name)
            first = build(temp_dir / "first.pdf")
            second = build(temp_dir / "second.pdf") if args.verify_deterministic else None
            if second is not None and first.read_bytes() != second.read_bytes():
                raise SystemExit("PDF_DETERMINISM_FAILED")
            if args.check:
                target = args.output.resolve()
                if not target.is_file() or first.read_bytes() != target.read_bytes():
                    raise SystemExit(f"PDF_OUT_OF_DATE: {target}")
            print(f"PDF_SHA256={sha256_file(first)}")
    else:
