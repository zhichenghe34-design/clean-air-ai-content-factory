...
def probe_preinstalled_runtimes(
    ...
    for code, command, cwd, environment in probes:
        try:
            result = runner(
                command,
                cwd=str(cwd),
...
def build_app_environment(
    base: Mapping[str, str], config: LauncherConfig, *, app_port: int, mpt_port: int
) -> dict[str, str]:
    environment = {
        key: value
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        probe_preinstalled_runtimes(config)
