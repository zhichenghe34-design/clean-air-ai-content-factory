...
(async () => {
  ...
  let detailJob = {
    ...
    review_policy: { ...MECHANICAL_POLICY },
  ...
  await page.route('**/api/jobs/fake-job/run', async route => {
    ...
    try {
      calls.push(runRequests === 1 ? 'run-long' : `run-${runRequests}`);
      runCount += 1;
      runInFlight = true;
      const completedRunId = runRequests >= 2 ? 'render-run-5' : 'render-run-4';
      job = {
        ...job,
        status: 'rendering',
        active_run_id: completedRunId,
        runs: [...(job.runs || []), { run_id: completedRunId, stage: 'render', status: 'running' }],
      };
      ...
      job = {
        ...
        approvals: {
          research: { status: 'approved', ...MECHANICAL_IDENTITY },
          compliance: { status: 'approved', ...MECHANICAL_IDENTITY },
        },
        runs: (job.runs || []).map(run => run.run_id === completedRunId ? { ...run, status: 'complete' } : run),
      };
      return route.fulfill({ status: 200, contentType: 'application/json; charset=utf-8', body: JSON.stringify(job) });
    } finally {
      runInFlight = false;
  ...
  await page.route('**/api/jobs/fake-job/approvals/research', async route => {
    ...
    job = {
      ...
      approvals: {
        ...job.approvals,
        research: { status: 'approved', ...MECHANICAL_IDENTITY, note: researchApprovalPayload.note },
      },
    };
    return route.fulfill({ status: 200, contentType: 'application/json; charset=utf-8', body: JSON.stringify(job) });
  });
  await page.route('**/api/jobs/fake-job/approvals/compliance', async route => {
    ...
    job = {
      ...
      approvals: {
        ...job.approvals,
        compliance: { status: 'approved', ...MECHANICAL_IDENTITY, note: complianceApprovalPayload.note },
      },
    };
    return route.fulfill({ status: 200, contentType: 'application/json; charset=utf-8', body: JSON.stringify(job) });
  });
  ...
  const customerOneClickCompleted = runRequests === 1
    && researchApprovalPayload === null
    && complianceApprovalPayload === null
    && await page.locator('#activeJobPanel [data-home-action="approve-research"]').count() === 0
    && await page.locator('#activeJobPanel [data-home-action="approve-compliance"]').count() === 0;
  await page.locator('#appMenuButton').click();
  await page.locator('#appMenu [data-view="jobs"]').click();
  await page.locator('#jobList button[data-action="open"][data-job-id="fake-job"]').click();
  ...
  await page.locator('#appMenuButton').click();
  await page.locator('#appMenu [data-view="jobs"]').click();
  job = { ...job, review_policy: { ...HUMAN_POLICY } };
  ...
  const internalReviewControlsAbsent = await page.locator('#researchFindings, #researchApprovalPanel, #complianceApprovalPanel').count() === 0;
  ...
  const diagnosticHistorySummary = diagnosticHistoryPresent
    ...
    : '';
  const diagnosticHistoryExplainsFailures = diagnosticHistorySummary.includes('运行记录')
  ...
  const expectedCalls = ['create-network-uncertain', 'create-replay', 'authorize-network-uncertain', 'run-long', 'run-2'];
  const createReplayReusedKey = createKeys[0] && createKeys[0] === createKeys[1];
  const noAutomaticRunReplay = runRequestsBeforeRevision === 1;
  const logicalAttemptKeysAreUnique = runKeys.length === 2 && new Set(runKeys).size === 2;
  const humanImpersonationAbsent = job.approvals.research?.human_approval_claimed === false
  ...
  const result = { status: job.status, calls, expectedCalls, customerOneClickCompleted, createKeys, createReplayReusedKey, createProductionMode, latestDownloadAvailable, completedHumanActionsVisible, deliveryHintIsOperatorFriendly, technicalArtifactsCollapsed, rawManifestOnlyInTechnicalAttachments, rawManifestHiddenByDefault, revisionCurrentScriptVisible, revisionExactEditGuidanceVisible, revisionExactContract, revisionTriggeredExactlyOneRun, revisionPatchRequests, legacyCorrectionRequests, revisionRunRequests, nonMechanicalCompletionHidesExactRevision, evidenceReportRequests, evidenceReportOpenedAsHumanHtml, reportContentType, publicEvidenceRequests, zipDownloadNamingCorrect, expectedZipFilename, authorizeRequests, runKeys, noAutomaticRunReplay, logicalAttemptKeysAreUnique, maxConcurrentRunRequests, pollsDuringLongRun, injectedPollFailures, recoveredPolls, detailJobReads, detailArtifactRequests, detailRunRequests, internalReviewControlsAbsent, mechanicalRevisionJobReads, mechanicalRevisionArtifactRequests, mechanicalTaskHasNoManualRun, mechanicalReviewIsAutomatic, mechanicalProviderSourceIsExplicit, mechanicalHumanControlsHidden, listRetryVisible, detailRetryButtonVisible, detailRetryButtonText, detailRetryButtonState, detailJobSummaryText, detailRetryVisible, diagnosticHistoryPresent, diagnosticHistoryCollapsed, diagnosticHistorySummary, diagnosticHistoryExplainsFailures, failedRunDetailsHidden, mechanicalScrollBefore, mechanicalScrollAfter, mechanicalScrollStable, mechanicalTerminalPollingStopped, retryRequests, retryReplayReusedKey, retryFailureReasonVisible, sameTopicRetryVisible, retryCreatedFreshJob, retryTriggeredExactlyOneRun, completedOpenEnabled, completedDetailUpdated, humanImpersonationAbsent, expectedFailureSignals: errors, unexpectedErrors };
  process.stdout.write(JSON.stringify(result));
  await context.close();
  await browser.close();
  if (unexpectedErrors.length || job.status !== 'complete' || JSON.stringify(calls) !== JSON.stringify(expectedCalls) || !customerOneClickCompleted || createRequests !== 2 || !createReplayReusedKey || createProductionMode !== 'motion' || !latestDownloadAvailable || !completedHumanActionsVisible || !deliveryHintIsOperatorFriendly || !technicalArtifactsCollapsed || !rawManifestOnlyInTechnicalAttachments || !rawManifestHiddenByDefault || !revisionCurrentScriptVisible || !revisionExactEditGuidanceVisible || !revisionExactContract || !revisionTriggeredExactlyOneRun || !nonMechanicalCompletionHidesExactRevision || !evidenceReportOpenedAsHumanHtml || !zipDownloadNamingCorrect || authorizeRequests !== 1 || !noAutomaticRunReplay || !logicalAttemptKeysAreUnique || maxConcurrentRunRequests !== 1 || pollsDuringLongRun < 2 || injectedPollFailures !== 1 || recoveredPolls < 1 || detailJobReads < 3 || detailArtifactRequests < 1 || detailRunRequests !== 0 || !internalReviewControlsAbsent || !mechanicalTaskHasNoManualRun || !mechanicalReviewIsAutomatic || !mechanicalProviderSourceIsExplicit || !mechanicalHumanControlsHidden || !listRetryVisible || !detailRetryVisible || !diagnosticHistoryPresent || !diagnosticHistoryCollapsed || !diagnosticHistoryExplainsFailures || !failedRunDetailsHidden || !mechanicalScrollStable || !mechanicalTerminalPollingStopped || retryRequests !== 2 || !retryReplayReusedKey || !retryFailureReasonVisible || !sameTopicRetryVisible || !retryCreatedFreshJob || !retryTriggeredExactlyOneRun || !completedOpenEnabled || !completedDetailUpdated || !humanImpersonationAbsent) process.exit(1);
})();
