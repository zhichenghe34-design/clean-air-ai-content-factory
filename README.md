...
Python 测试由 `unittest discover` 动态发现，当前 125 项 Python 测试同时覆盖 v2 状态/审批/预算/并发/密钥回归，以及 v3 动态行业能力包、不可变注册表、来源信任、通用生产、纠错记忆和 Skill 晋升。浏览器烟雾测试继续覆盖 Provider 三态、“恰好 3 个候选、恰好 1 个选中”、换一批、自定义输入、两道人工作业门禁、窄屏无溢出与零前端错误。CI 使用可注入的假配音/渲染适配器完成快速确定性 E2E。
