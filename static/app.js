...
function renderRunningCard(title, message, progress) {
  return `<div class="job-chat-head"><div><h2>${escapeHtml(title)}</h2><p>Agent 会在需要你本人确认时停下来。</p></div><span class="status-pill status-running">处理中</span></div>
...
async function approveHomeCompliance(jobId) {
  ...
  await api(`/api/jobs/${jobId}/approvals/compliance`, { method: "POST", body: JSON.stringify({
    decision: "approved", reviewer: "本机会话用户", note: "通过 Agent 首页确认开始渲染", artifact_sha256: review.sha256, script_sha256: script.sha256,
  }) });
...
function renderLatestArtifact() {
  ...
  target.innerHTML = `<video id="latestVideo" class="latest-video" src="/api/jobs/${latest.id}/artifacts/final.mp4" preload="metadata" playsinline></video>
    <div class="latest-meta"><h3>${escapeHtml(latest.production_input?.topic || latest.id)}</h3><div class="latest-status"><img class="ui-icon" src="/icons/check.svg" alt="">已完成 · 合规已确认 · <span id="latestDuration">00:--</span></div>
...
function renderResearchFindings(findings, strictAudit = null) {
  ...
  const rows = findings.map(item => {
    ...
    if (item.auto_review_status === "eligible") {
      return `<div class="finding" data-finding-id="${escapeHtml(item.finding_id)}" data-evidence-type="paraphrase" data-decision="approved"><span class="review-recommendation">严格审核 Agent：仅限安全转述</span><strong>${escapeHtml(guide.summary)}</strong><dl class="review-guide"><dt>可以怎么说</dt><dd>${escapeHtml(guide.allowed)}</dd><dt>绝对不能怎么说</dt><dd>${escapeHtml(guide.prohibited)}</dd></dl><div class="finding-control"><label>你的决定<select data-field="decision"><option value="approved">采用安全转述</option><option value="rejected">这条不用</option></select></label></div><details><summary>查看原文和来源</summary><p>${escapeHtml(guide.source)}</p>${excerpts ? `<ul class="evidence-list">${excerpts}</ul>` : ""}<small>${escapeHtml(sources)}</small></details></div>`;
    }
